...
import {
    ...
    updateCommentDiscussion,
    replyCommentDiscussion,
    deleteCommentDiscussion
} from '../api/comments';
...
export const useArtworkComments = (artID, userID, TargetUserIDOfArtwork, commentID) => {
    ...
    console.log(TargetUserIDOfArtwork);
...
export const useDiscussionComments = (discussionID, userID, TargetUserIDOfDiscussion, commentID) => {
    const [comments, setComments] = useState([]);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    ...
    const createComment = useCallback(
        async (content) => {
            if (!userID || !discussionID) {
                setCreateError('UserID and discussionID are required.');
                return;
            }
            if (!content.trim()) {
                setCreateError('Content cannot be empty.');
            ...
            socket.emit('sendNotification', {
                targetUserID: TargetUserIDOfDiscussion,
                contentID: discussionID,
                type: 'comment-discussion',
                link_url: `/discussion/${discussionID}`
            });
            ...
            try {
                const response = await createCommentDiscussion(userID, discussionID, content);
                if (response.error) {
                    setCreateError(response.error);
                } else {
                    setComments((prev) => [response.data, ...prev]); // Thêm comment vào danh sách
        ...
        [userID, discussionID, discussionID]
    );

    // Fetch comments
    const fetchComments = useCallback(async () => {
        if (!discussionID) {
            setError('discussionID is required.');
            return;
        }
        ...
        try {
            const response = await getCommentDiscussion(discussionID);
            if (response.error) {
                setError(response.error);
                setComments([]);
            } else {
    ...
    const updateComment = useCallback(
        async (commentID, content) => {
            ...
            setCreateError(null);
            try {
                const response = await updateCommentDiscussion(commentID, content);
                if (response.error) {
                    setCreateError(response.error);
                } else {
                    // Cập nhật lại nội dung comment trong state bằng hàm đệ quy
    ...
    const replyComment = useCallback(
        async (parentCommentID, content) => {
            if (!userID || !commentID || !parentCommentID) {
                setReplyError('UserID, commentID, and ParentCommentID are required.');
                return;
            }
            if (!content.trim()) {
                setReplyError('Content cannot be empty.');
            ...
            try {
                const response = await replyCommentDiscussion(parentCommentID, discussionID, userID, content);
        ...
        [userID, discussionID, TargetUserIDOfDiscussion]
    );
    ...
    const deleteComment = useCallback(async (commentID) => {
        ...
        setDeleteError(null);
        try {
            const response = await deleteCommentDiscussion(commentID);
            if (response?.error) {
                setDeleteError(response.error);
            } else {
                setComments((prevComments) => {
    ...
    useEffect(() => {
        fetchComments();
    }, [TargetUserIDOfDiscussion, fetchComments]);
